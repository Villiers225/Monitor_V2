...
def sim_hash(text: str) -> str:
    # Simple content hash to deduplicate
...
def score_article(meta, text, cfg):
    ...
    # Title keyword hits
    base_kw = ["defence procurement","defense procurement","acquisition","tender","contracting","DE&S","industrial base","NAO"]
    for k in base_kw:
        if k in title:
            score += 0.10
...
def extract_solutions(text):
    # Heuristic: sentences containing strong recommendation verbs
...
def simple_summary(text, n_sent=5):
    # Extractive: pick first N informative sentences
    if not text:
        return ""
...
def openai_summary(text, api_key, max_tokens=300):
    try:
        ...
        prompt = f"Summarise the following article in 5-7 bullet points focusing on defence procurement problems and proposed solutions. Be specific and concise.\n\n{text[:12000]}"
...
def gather_social(cfg):
    items = []
    try:
        # snscrape via subprocess for portability on Actions
...
def bias_terms_from_user(texts, topk=25):
    # Simple TF scoring to extract boosted terms
    freq = Counter()
    for t in texts:
...
def main():
    ...
    # Merge bias into config keywords (lightly)
    for term in boosted:
